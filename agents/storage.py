...
class RolloutStorage(object):
    ...
    def insert(self, obs, recurrent_hidden_states, actions, action_probs, action_log_probs,
                value_preds, q_preds, rewards, masks, bad_masks):
        ...
        self.q_preds[self.step].copy_(q_preds)
    ...
    def compute_returns(self,
        ...
        if use_proper_time_limits:
            raise Exception("Proper time limits not implemented")
            if use_gae:
            ...
            if use_gae:
                raise Exception("GAE not implemented")
                self.value_preds[-1] = next_value
                gae = 0
                for step in reversed(range(self.rewards.size(0))):
                    delta = self.rewards[step] + gamma * self.value_preds[
